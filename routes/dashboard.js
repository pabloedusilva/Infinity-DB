...
router.post('/horarios-turnos', requireLogin, async(req, res) => {
    ...
    await db`DELETE FROM horarios_turnos WHERE dia_semana = ${dia_semana}`;
    for (const t of turnos) {
        if (t.inicio && t.fim) {
            await db`INSERT INTO horarios_turnos (dia_semana, turno_inicio, turno_fim) VALUES (${dia_semana}, ${t.inicio}, ${t.fim})`;
        }
    }
...
router.post('/notificacoes', async(req, res) => {
    ...
    try {
        await db`INSERT INTO notificacoes (titulo, mensagem, data) VALUES (${titulo}, ${mensagem}, NOW())`;
        res.json({ success: true });
    } catch (err) {
...
router.delete('/notificacoes/:id', requireLogin, async(req, res) => {
    await db`DELETE FROM notificacoes WHERE id = ${req.params.id}`;
    res.json({ success: true });
});
...
router.get('/servico-imagens', async(req, res) => {
    try {
        const rows = await db`SELECT * FROM servico_imagens WHERE ativo = TRUE`;
